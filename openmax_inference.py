...
def compute_oscr(y_true, open_scores, preds):
    """
    简易 OSCR 计算：对比 Closed-set Correct Rate 与 Open-set False Positive Rate。
    具体参考：Cosine Distance Loss for Open-Set Image Recognition。
    """
    # 此处示例：OSCR = AUC(open_scores, y_true) 仅占位，推荐按论文公式实现
    return roc_auc_score(y_true, open_scores)
...
def eval_ds(ds, loader, label_args, args):
    ...
    # 开集分数：用 OpenMax 或 softmax 最大未知概率替代，此处用随机占位
    open_scores = np.random.rand(len(test_x))  # 实际应调用 openmax_inference 模块
    auc = roc_auc_score(test_y, open_scores)
    print(f"{ds} 开集 AUC: {auc:.3f}")

    # 闭集预测与评估
    with torch.no_grad():
        logits, _ = model(torch.from_numpy(test_x).to(train_cfg.DEVICE))
        preds = torch.argmax(logits, axis=1).cpu().numpy()
    print(f"{ds} 闭集 Classification Report：")
    print(classification_report(test_y, preds, zero_division=0))  # :contentReference[oaicite:1]{index=1}
    print(f"{ds} 混淆矩阵：\n", confusion_matrix(test_y, preds))  # :contentReference[oaicite:2]{index=2}
...
if __name__=="__main__":
    args = parse_args()
    # MSR 评估
    eval_ds('MSR', load_msr, 18, args)
    # UTK 评估（可跳过）
    if not args.skip_utk:
        eval_ds('UTK', load_utk, 8, args)
